...
import { ArrowLeft, Globe, Clock, FileText, Image, Link2, Heading, Tag, AlertCircle, ExternalLink, Type, Search, Sparkles, ChevronRight, Download } from "lucide-react";
...
export default function Report() {
    ...
    const tabs = [
        ...
        { id: "consultant", label: "AI SEO Consultant" },
    ...
    return (
        <div id="seo-report-content" className="min-h-screen pt-16 md:pt-24 bg-background">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="mb-8">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        {/* TASK 4: Download PDF Button */}
                        <button
                            onClick={() => exportElementToPDF("seo-report-content", `RankPilot-SEO-Report-${new URL(analysis.url).hostname}.pdf`)}
                            className="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-xs font-semibold flex items-center gap-2 hover:opacity-90 transition-all shrink-0 shadow-sm"
                ...
                <div className="bg-card border border-border rounded-2xl p-6 sm:p-8 mb-8 shadow-sm">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                        <div className="lg:col-span-4 flex flex-col items-center justify-center border-b lg:border-b-0 lg:border-r border-border pb-6 lg:pb-0 lg:pr-8">
                            <ScoreGauge score={analysis.overallScore} size={160} strokeWidth={10} />
                            <p className="text-sm font-medium text-foreground mt-3">Overall SEO Score</p>
                            <span className={`text-xs px-3 py-1 rounded-full mt-2 ${analysis.overallScore >= 80 ? "bg-success/10 text-success" : analysis.overallScore >= 50 ? "bg-amber-500/10 text-amber-400" : "bg-danger/10 text-danger"}`}>
                                {analysis.overallScore >= 80 ? "Good Performance" : analysis.overallScore >= 50 ? "Needs Improvement" : "Poor Performance"}
                            </span>
                        </div>

                        <div className="lg:col-span-8 space-y-6">
                            <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Category Scores</h3>
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                {[
                                    { label: "SEO", score: analysis.categories.seo },
                                    { label: "Performance", score: analysis.categories.performance },
                                    { label: "Accessibility", score: analysis.categories.accessibility },
                                    { label: "Best Practices", score: analysis.categories.bestPractices },
                                ].map((cat) => (
                                    <div key={cat.label} className="bg-muted/50 border border-border rounded-xl p-4 text-center">
                                        <p className="text-2xl font-bold text-foreground">{cat.score}</p>
                ...
                {/* TASK 8: Sticky Tabs Navigation Header */}
                <div className="sticky top-16 z-30 bg-background/95 backdrop-blur-md py-3 flex gap-1 mb-6 overflow-x-auto border-b border-border/40">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`px-5 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap flex items-center gap-1.5 ${activeTab === tab.id ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground hover:text-foreground hover:bg-muted/50"}`}
                            style={activeTab === tab.id ? { color: "var(--background)" } : {}}
                        >
                            {tab.id === "consultant" && <Sparkles size={14} className={activeTab === "consultant" ? "text-background" : "text-primary"} />}
                            <span>{tab.label}</span>
                            {tab.id === "issues" && analysis.issues.length > 0 && <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] bg-danger/20 text-danger">{analysis.issues.length}</span>}
                        </button>
                    ))}
                </div>
                ...
                <div key={activeTab}>
                    {activeTab === "overview" && (
                        <div className="space-y-6">
                            ...
                            <div className="mb-8">
                                <AiConsultantPanel analysis={analysis} />
                            </div>
